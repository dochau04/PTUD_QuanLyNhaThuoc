...
public class BaoCaoThongKe_GUI extends JFrame {
	...
	private JComponent cbbKhachHang;
	private JLabel lblNhanVien;
	private Box boxNhanVien;
	private JComponent cbbNhanVien;
	private Box boxSapXep;
	private JComponent lblThongKe;
	...
	public void TrangChuQuanLy_GUI() {
		...
		panel_Full.setComponentZOrder(panel_Menu, 0);
		...
		// ===== Khởi tạo layout tổng =====
		boxBoLoc = Box.createVerticalBox();

		// ==== HÀNG 1: Năm – Tháng – Ngày ====
		...
		cbbThang.setLightWeightPopupEnabled(false);
		...
		cbbNgay.setLightWeightPopupEnabled(false);
		...
		// Thêm vào hàng 1
		boxBoLoc1.add(Box.createHorizontalGlue());
		...
		// ==== HÀNG 2: Nhân viên – Thống kê theo (CĂN GIỮA) ====
		...
		boxNhanVien.add(Box.createHorizontalStrut(5));
		...
		boxSapXep.add(Box.createHorizontalStrut(5));
		boxSapXep.add(cbbSapXep);

		// Thêm vào hàng 2 (CĂN GIỮA)
		boxBoLoc2.add(Box.createHorizontalGlue());
		boxBoLoc2.add(boxNhanVien);
		boxBoLoc2.add(Box.createHorizontalStrut(40));
		...
		boxBoLocBtn.setAlignmentX(Component.CENTER_ALIGNMENT);
		

		// ==== GỘP TẤT CẢ VÀO boxBoLoc ====
		...
		// ==== Thêm vào panel_Center nếu chưa có ====
		if (boxBoLoc.getParent() == null) {
			panel_Center.add(boxBoLoc);
		}
	...
	private void initComponents() 
	{
		...
		filterPanel.setLayout(new FlowLayout(FlowLayout.CENTER, 15, 8));
		filterPanel.setBackground(Color.WHITE);
		panel_Center.add(filterPanel);
		
		filterPanel.add(dateFrom);
		filterPanel.add(dateTo);
		filterPanel.add(cboLoaiThongKe);
		filterPanel.add(cboSanPham);
	...
	private void resizePanel() {
		...
		if (boxBoLoc != null) {
		    ...
		    int BoxWidth = 1000;
		    int BoxHeight = 250;
		    int boxX = (panel_Center.getWidth() - BoxWidth) / 2;
		    int boxY = 0;
		    boxBoLoc.setBounds(boxX, boxY, BoxWidth, BoxHeight);
        ...
        // Gắn sự kiện click để test in ra giá trị được chọn
...
//		
		...
		if (scrollPane == null) {
			// Dữ liệu mẫu
			String[] columnNames = { "Mã HĐ", "Ngày lập", "Tên khách hàng", "Tổng tiền", "Chi Phí", "Lợi Nhuận" };
//            Object[][] data = {
...
//            };
		...
		int x_Table = 0;
		int y_Table = (int) (panel_Center.getHeight() * 0.3);
		scrollPane.setBounds(0, y_Table, panel_Center_Width, panel_Center_Height - y_Table - 10);
		int height = 800;
